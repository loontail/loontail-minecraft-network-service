...
use crate::join_requests::{are_friends, fetch_user_dto, issue_ticket_and_relay, JoinTicketDto};
...
async fn user_exists(pool: &PgPool, id: Uuid) -> AppResult<bool> {
    Ok(sqlx::query_scalar("SELECT EXISTS (SELECT 1 FROM users WHERE id = $1)")
        .bind(id)
        .fetch_one(pool)
        .await?)
}

async fn list_by(state: &AppState, query: &str, user_id: Uuid) -> AppResult<Json<Vec<WorldInviteDto>>> {
...
pub async fn create(
    ...
    if invitee == host {
        return Err(AppError::BadRequest("the host is already in the world".into()));
    }
...
pub async fn decline(
    ...
    sqlx::query("UPDATE world_invites SET status = 'declined', updated_at = now() WHERE id = $1")
        ...
        .execute(&state.pool)
        .await?;
    Ok(Json(build_dto(&state.pool, load_invite(&state.pool, id).await?).await?))
}
