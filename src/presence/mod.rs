...
use crate::models::{UserDto, UserStatus};
...
pub async fn heartbeat(
    ...
    crate::metrics::Metrics::incr(&state.metrics.heartbeats);
...
pub async fn friends_with_presence(state: &AppState, user_id: Uuid) -> AppResult<Vec<FriendPresence>> {
    ...
    let friends = rows
        ...
        .map(|row| {
            ...
            let (status, current_world_session_id) = match row.guest_world_id {
                Some(world) if base != UserStatus::Offline => {
