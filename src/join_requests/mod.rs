...
use crate::models::{User, UserDto, UserStatus};
...
pub(crate) async fn issue_ticket_and_relay(
    ...
    let ttl = Duration::from_std(state.config.join_ticket_ttl)
    ...
    tx.commit().await?;
    crate::metrics::Metrics::incr(&state.metrics.join_tickets_issued);
...
pub async fn create_join_ticket(
    ...
    if host == guest {
        return Err(AppError::BadRequest("you cannot join your own world".into()));
    }
...
pub async fn create_join_request(
    ...
    let ttl = Duration::from_std(state.config.join_request_ttl)
        .unwrap_or_else(|_| Duration::seconds(60));
    let expires_at = Utc::now() + ttl;
    ...
    let dto = build_join_request_dto(&state.pool, load_join_request(&state.pool, id).await?).await?;
