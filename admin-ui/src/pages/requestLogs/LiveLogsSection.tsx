...
import { TableSkeletonRows } from "@/components/shared/TableStates";
...
import { useRequestLogTail } from "@/features/requestLogs/api";
import { formatLatency } from "@/shared/lib/chartFormat";
  ...
  return (
    <Card>
      ...
      <CardContent className="px-0">
        ...
        ) : !tail.isLoading && entries.length === 0 ? (
          <div className="px-6">
            <EmptyState
              icon={ScrollText}
          ...
          <Table>
            ...
            <TableBody striped>
              {tail.isLoading ? (
                ...
                entries.map((entry, i) => (
                  <LogRow
                    // biome-ignore lint/suspicious/noArrayIndexKey: ring-buffer entries have no id; ts+i is stable within a snapshot and rows hold no local state.
                    key={`${entry.ts}-${i}`}
                    entry={entry}
                  />
                ))
