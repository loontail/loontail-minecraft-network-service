...
import {
  useRequestLogSummary,
  useRequestLogTimeseries,
...
import type { RequestMetric, StatusClass, TimeRange } from "@/shared/types";
...
export function TrafficSection({ range }: { range: TimeRange }) {
  const [metric, setMetric] = useState<RequestMetric>("requests");
  const summary = useRequestLogSummary(range);
  const series = useRequestLogTimeseries(range, metric);
    ...
    <section className="flex flex-col gap-4" aria-label="Traffic dashboard">
        ...
        <Card className="xl:col-span-2">
          <CardHeader className="flex flex-row items-start justify-between gap-4">
            <div className="flex flex-col gap-1.5">
              ...
              <CardDescription>
                Bucketed over the selected range.
              </CardDescription>
            </div>
          ...
          <CardContent>
            {series.isLoading ? (
              ...
              <EmptyChartState
                ...
                detail="The request failed. Try another range or metric."
              />
            ) : !hasSeries ? (
              <EmptyChartState
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Most-requested routes in the selected range.
          </CardDescription>
        </CardHeader>
        <CardContent className="px-0">
          {summary.isLoading ? (
            ...
            <p className="px-6 py-6 text-center text-body text-text-mute">
              No path activity in this range yet.
            </p>
          ) : (
            <Table>
              <TableHeader>
